...
import { DatePickerPro, showDatePickerPro } from "@dyb-dev/vant-pro"
...
const data: IData = reactive({
    pickerValue: [
        dayjs().year().toString(), // 当前年份转为字符串
        (dayjs().month() + 1).toString(), // 当前月份转为字符串
        dayjs().date().toString() // 当前日期转为字符串
    ],
    selectResult: null
})
